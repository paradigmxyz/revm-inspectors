...
fn test_geth_calltracer_top_call_reverting() {
    ...
    */
    ...
    let deployer = Address::ZERO;
    let addr = deploy_contract(&mut evm, code.into(), deployer, SpecId::LONDON).created_address().unwrap();
    ...
    let addr2 = deploy_contract(&mut evm2, code.into(), deployer, SpecId::LONDON).created_address().unwrap();
    ...
    // nestedEmitWithFailureAfterNestedEmit calls doubleNestedEmitWithSuccess which calls nestedEmitWithSuccess
    // So we should see nested calls when only_top_call = false
    assert_eq!(call_frame_all.calls.len(), 1, "Should have one subcall for doubleNestedEmitWithSuccess");
    ...
    assert_eq!(call_frame_all.calls[0].calls.len(), 1, "doubleNestedEmitWithSuccess should call nestedEmitWithSuccess");
}
...
fn test_geth_calltracer_nested_revert() {
    ...
    */
    ...
    assert!(call_frame_all.error.is_some(), "Top call should have an error");
    assert!(call_frame_all.calls[0].error.is_some(), "nestedEmitWithFailure should also have an error");
    
    // Test revert with topcall
    let mut insp3 = TracingInspector::new(TracingInspectorConfig::default_geth().set_record_logs(true));
    let mut evm3 = Context::mainnet()
        .with_db(CacheDB::new(EmptyDB::default()))
        .build_mainnet();
    
    let addr3 = deploy_contract(&mut evm3, code.into(), deployer, SpecId::LONDON).created_address().unwrap();
    ...
    // nestedEmitWithFailure emits a log before reverting, but since it reverts, the log should not be included
